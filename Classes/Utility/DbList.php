    ...
    public function getTRsysFolderContent(SimpleXMLElement &$xmlNode, $uid, $depth, $t3mind = NULL)
    {
        ...
        foreach ($this->tablesInSysFolder as $tableName => $values)
        {
            ...
            foreach ($values as $k => $row)
            {
                ...
                $attr = array(
                    'ID' => 'tISF139v',
                    'TEXT' => (isset($row['titInt0']) ? $row['titInt0'] : 'No internal title set @ DbList.php ' . __LINE__)
                ...
                if ($this->parentObject->isMapModeBE())
                {
                    $attr['LINK'] = $this->parentObject->getBEHttpHost() .
                        'typo3/alt_doc.php?edit[' . $tableName . '][' . $row['uid'] . ']=edit';
                }
...
//                        ":\n<br>\n" . mysql_error() . "<hr>" . var_export($queryParts, 1));
            ...
            elseif( !$result ) {
                // SQL error Bug #38159
                // user has to check his/her TCA and table config ... :-(
                $this->tablesInSysFolder[$tableName] = array(
                    'TotalItems' => mysql_error(),
                );
            }
        }/* endforeach */
    }
    ...
    private function _getTotalItems($queryParts)
    {
        return $GLOBALS['TYPO3_DB']->exec_SELECTcountRows(
                        '*', $queryParts['FROM'], $queryParts['WHERE']
        );
    }
        ...
        if (is_array($TCA[$table]))
        {
            ...
            foreach ($this->addFieldsDependedIfTheyAreSetOrNot as $k => $column)
            {
                ...
                if (!is_array($column) && isset($tcaCurrent['ctrl'][$column]) && !empty($tcaCurrent['ctrl'][$column]))
                {
                    ...
                    if ($column == 'label')
                    { /* column must exists in the table */
